...
function HomePage() {
  ...
  // Fetch Root Photos
  useEffect(() => {
    const fetchRootPhotos = async () => {
      const result = await getRootPhotos(uid);
      setPhotos(result);
    };
    fetchRootPhotos().catch(() => setPhotos([]));
  }, [uid]);

  // Fetch Albums
  useEffect(() => {
    const fetchAlbums = async () => {
      const result = await getAlbums(uid);
      setAlbums(result);
    };
    fetchAlbums().catch(() => setAlbums([]));
  }, [uid]);
